...
void ASoulsLikeCharacter::Jump(const FInputActionValue& Value)
{
	bool IsAttacking = CurrentPlayerState == EPlayerState::Attacking;

	if (Value.Get<bool>() && !IsAttacking)
	{
		ACharacter::Jump();
	}
}
...
bool ASoulsLikeCharacter::IsCurrentStateIncluding(TArray<EPlayerState> States)
{
	return States.Contains(CurrentPlayerState);
}

bool ASoulsLikeCharacter::CanAttack()
{
	bool isFalling = GetCharacterMovement()->IsFalling();
	bool isAttacking = IsCurrentStateIncluding({ EPlayerState::Attacking });


	return !isFalling && !isAttacking;
}

void ASoulsLikeCharacter::SaveLightAttack()
{
	if (!bSaveLightAttack) {
		return;
...
void ASoulsLikeCharacter::ResetLightAttack()
{
	bSaveLightAttack = false;
	LightAttackIndex = 0;
}

void ASoulsLikeCharacter::PerformLightAttack(int AttackIndex)
{
	int lightAttackMontagesLength = LightAttackMontages.Num();

	if (AttackIndex >= lightAttackMontagesLength)
	{
		LightAttackIndex = 0;
	}

	UAnimMontage* LightAttackMontage = LightAttackMontages[LightAttackIndex];

	if (!LightAttackMontage)
	{
		/** LOG some ERROR */
		UE_LOG(LogTemplateCharacter, Error, TEXT("Light Attack Montage is not set!"));
		return;
	}
	...
	if (LightAttackIndex >= lightAttackMontagesLength)
	{
		LightAttackIndex = 0;
	}
}